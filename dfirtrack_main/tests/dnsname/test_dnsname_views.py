...
class DnsnameViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        User.objects.create_user(
            username='testuser_dnsname', password='TeWFLE2k6lqoC7c6xc0x'
        )
    ...
    def test_dnsname_detail_not_logged_in(self):
        ...
        destination = '/login/?next=' + urllib.parse.quote(
            '/dnsname/' + str(dnsname_1.dnsname_id) + '/', safe=''
        )
        # get response
        response = self.client.get(
            '/dnsname/' + str(dnsname_1.dnsname_id) + '/', follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
    ...
    def test_dnsname_add_post_redirect(self):
        ...
        # get object
        dnsname_id = Dnsname.objects.get(
            dnsname_name='dnsname_add_post_test'
        ).dnsname_id
        # create url
    ...
    def test_dnsname_edit_not_logged_in(self):
        ...
        # create url
        destination = '/login/?next=' + urllib.parse.quote(
            '/dnsname/' + str(dnsname_1.dnsname_id) + '/edit/', safe=''
        )
        # get response
        response = self.client.get(
            '/dnsname/' + str(dnsname_1.dnsname_id) + '/edit/', follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
    ...
    def test_dnsname_edit_post_redirect(self):
        ...
        response = self.client.post(
            '/dnsname/' + str(dnsname_1.dnsname_id) + '/edit/', data_dict
        )
        # get object
        dnsname_2 = Dnsname.objects.get(dnsname_name='dnsname_edit_post_test_2')
        # create url
        destination = urllib.parse.quote(
            '/dnsname/' + str(dnsname_2.dnsname_id) + '/', safe='/'
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
