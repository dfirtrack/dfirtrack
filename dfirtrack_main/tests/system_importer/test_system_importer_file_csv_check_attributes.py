...
def compare_messages_faulty_systems(self, messages):
    ...
    self.assertEqual(
        messages[0].message,
        'Value for system in row 2 was too long. System not created.',
    )
    self.assertEqual(messages[0].level_tag, 'warning')
    self.assertEqual(
        messages[1].message,
        'Value for system in row 4 was an empty string. System not created.',
    )
    self.assertEqual(messages[1].level_tag, 'warning')
    self.assertEqual(
        messages[2].message,
        'Index for system in row 5 was out of range. System not created.',
    )
...
def compare_messages_csv(self, messages):
    ...
    self.assertEqual(
        ...
        'Value for DNS name in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for domain in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for location in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for OS in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for reason in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for recommendation in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for serviceprovider in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for systemtype in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for IP address in row 2 was not a valid IP address.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for case in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for company in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Value for tag in row 2 was not a valid value.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Index for DNS name in row 3 was out of range.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message, 'Index for domain in row 3 was out of range.'
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Index for location in row 3 was out of range.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message, 'Index for OS in row 3 was out of range.'
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message, 'Index for reason in row 3 was out of range.'
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Index for recommendation in row 3 was out of range.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Index for serviceprovider in row 3 was out of range.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Index for systemtype in row 3 was out of range.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message, 'Index for IP in row 3 was out of range.'
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message, 'Index for case in row 3 was out of range.'
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message,
        'Index for company in row 3 was out of range.',
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
    self.assertEqual(
        messages[message_counter].message, 'Index for tag in row 3 was out of range.'
    )
    self.assertEqual(messages[message_counter].level_tag, 'warning')
    message_counter += 1
...
def compare_system_and_attributes_csv(self):
    ...
    # compare - systems / attributes
    self.assertTrue(
        System.objects.get(system_name='system_csv_32_001')
        .ip.filter(ip_ip='127.32.1.1')
        .exists()
    )
    self.assertTrue(
        System.objects.get(system_name='system_csv_32_001')
        .ip.filter(ip_ip='127.32.1.2')
        .exists()
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').dnsname,
        Dnsname.objects.get(dnsname_name='dnsname_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').domain,
        Domain.objects.get(domain_name='domain_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').location,
        Location.objects.get(location_name='location_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').os,
        Os.objects.get(os_name='os_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').reason,
        Reason.objects.get(reason_name='reason_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').recommendation,
        Recommendation.objects.get(recommendation_name='recommendation_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').serviceprovider,
        Serviceprovider.objects.get(serviceprovider_name='serviceprovider_1'),
    )
    self.assertEqual(
        System.objects.get(system_name='system_csv_32_001').systemtype,
        Systemtype.objects.get(systemtype_name='systemtype_1'),
    )
    self.assertTrue(
        System.objects.get(system_name='system_csv_32_001')
        .case.filter(case_name='case_1')
        .exists()
    )
    self.assertTrue(
        System.objects.get(system_name='system_csv_32_001')
    ...
    self.assertTrue(
        System.objects.get(system_name='system_csv_32_001')
        .tag.filter(tag_name='AUTO_tag_1')
        .exists()
    )
...
def compare_system_and_attributes_domain_name(self):
    ...
    self.assertEqual(
        System.objects.get(system_name='system_csv_33_001').domain,
        Domain.objects.get(domain_name='domain_33_1'),
    )
    self.assertEqual(System.objects.get(system_name='system_csv_33_002').domain, None)
    self.assertEqual(
        System.objects.get(system_name='system_csv_33_003').domain,
        Domain.objects.get(domain_name='domain_33_3'),
    )
...
class SystemImporterFileCsvCheckAttributesViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_user = User.objects.create_user(
            username='testuser_system_importer_file_csv_check_attributes',
            password='vlQnN2tg9HVGyyyIvezt',
        )
    ...
    def test_system_importer_file_csv_check_attributes_cron_faulty_systems(self):
        ...
        # change config
        set_csv_import_filename(
            'system_importer_file_csv_testfile_31_faulty_system.csv'
        )
        ...
        self.assertEqual(
            messages[0].message,
            'System CSV importer: created: 3 | updated: 0 | skipped: 3 | multiple: 0 [2021-03-08 18:05:00 - 2021-03-08 18:05:00]',
        )
        self.assertEqual(messages[0].level_tag, 'success')
        # switch user context
        self.client.logout()
    ...
    def test_system_importer_file_csv_check_attributes_upload_post_faulty_systems(self):
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                'dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_31_faulty_system.csv',
            )
        )
        # create post data
        data_dict = {
    ...
    def test_system_importer_file_csv_check_attributes_upload_post_faulty_attributes(
        self,
    ):
        """test importer view"""
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                'dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_32_faulty_attributes.csv',
            )
        )
        # create post data
        data_dict = {
    ...
    def test_system_importer_file_csv_check_attributes_cron_domain_name(self):
        ...
        self.assertEqual(
            messages[0].message,
            'System CSV importer: created: 3 | updated: 0 | skipped: 0 | multiple: 0 [2021-03-08 18:20:00 - 2021-03-08 18:20:00]',
        )
        self.assertEqual(messages[0].level_tag, 'success')
        # switch user context
        self.client.logout()
    ...
    def test_system_importer_file_csv_check_attributes_upload_post_domain_name(self):
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                'dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_33_domain_name.csv',
            )
        )
        # create post data
        data_dict = {
