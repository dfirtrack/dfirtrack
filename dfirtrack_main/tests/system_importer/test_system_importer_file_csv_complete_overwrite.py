...
def pre_compare_system_and_attributes_csv(self):
    ...
    self.assertFalse(
        Recommendation.objects.filter(
            recommendation_name='recommendation_41_1'
        ).exists()
    )
    self.assertFalse(
        Serviceprovider.objects.filter(
            serviceprovider_name='serviceprovider_41_1'
        ).exists()
    )
    self.assertFalse(
        Systemtype.objects.filter(systemtype_name='systemtype_41_1').exists()
    )
    ...
    self.assertFalse(
        system_csv_41_001.company.filter(company_name='company_41_1').exists()
    )
    self.assertFalse(
        system_csv_41_002.company.filter(company_name='company_41_1').exists()
    )
    self.assertFalse(
        system_csv_41_003.company.filter(company_name='company_41_1').exists()
    )
...
def compare_system_and_attributes_csv(self):
    ...
    recommendation_41_1 = Recommendation.objects.get(
        recommendation_name='recommendation_41_1'
    )
    serviceprovider_41_1 = Serviceprovider.objects.get(
        serviceprovider_name='serviceprovider_41_1'
    )
    ...
    self.assertFalse(
        system_csv_41_001.company.filter(company_name='company_1').exists()
    )
    self.assertFalse(
        system_csv_41_002.company.filter(company_name='company_1').exists()
    )
    self.assertFalse(
        system_csv_41_003.company.filter(company_name='company_1').exists()
    )
...
class SystemImporterFileCsvCompleteOverwriteViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_user = User.objects.create_user(
            username='testuser_system_importer_file_csv_complete_overwrite',
            password='EeD9m7nMmWTVeUeAAPUC',
        )
        User.objects.create_user(
            username='message_user', password='AUDrWv4ksLwPhgrDPZ6y'
        )
    ...
    def test_system_importer_file_csv_complete_overwrite_cron(self):
        ...
        # mock timezone.now()
        t_1 = datetime(2021, 3, 17, 19, 30, tzinfo=dttimezone.utc)
    ...
    def test_system_importer_file_csv_complete_overwrite_upload_post(self):
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                'dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_41_complete_overwrite.csv',
            )
        )
        # create post data
        data_dict = {
