...
def compare_tag_remove_all(self, file_number):
    ...
    self.assertFalse(
        ...
        .tag.filter(tag_name="AUTO_tag_96_1")
        .exists()
    )
    self.assertFalse(
    ...
    self.assertTrue(
        System.objects.get(system_name=f"system_csv_{file_number}_001")
        .tag.filter(tag_name=f"AUTO_tag_{file_number}_1")
        .exists()
    )
    self.assertTrue(
        System.objects.get(system_name=f"system_csv_{file_number}_001")
        .tag.filter(tag_name=f"AUTO_tag_{file_number}_2")
        .exists()
    )
    self.assertTrue(
        System.objects.get(system_name=f"system_csv_{file_number}_001")
        .tag.filter(tag_name=f"AUTO_tag_{file_number}_3")
        .exists()
    )
    self.assertTrue(
        System.objects.get(system_name=f"system_csv_{file_number}_002")
...
class SystemImporterFileCsvTagRemovalViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_user = User.objects.create_user(
            username="testuser_system_importer_file_csv_tag_removal",
            password="XAavYL75MrC5eVVSuzoL",
        )
        User.objects.create_user(
            username="message_user", password="8vwuoDthBxFkMQUBG2DM"
        )
    ...
    def test_system_importer_file_csv_cron_tag_remove_all(self):
        ...
        self.assertEqual(
            messages[0].message,
            "System CSV importer: created: 0 | updated: 3 | skipped: 0 | multiple: 0 [2021-03-26 18:35:00 - 2021-03-26 18:35:00]",
        )
        self.assertEqual(messages[0].level_tag, "success")
        # switch user context
        self.client.logout()
    ...
    def test_system_importer_file_csv_cron_tag_remove_prefix(self):
        ...
        self.assertEqual(
            messages[0].message,
            "System CSV importer: created: 0 | updated: 3 | skipped: 0 | multiple: 0 [2021-03-26 18:40:00 - 2021-03-26 18:40:00]",
        )
        self.assertEqual(messages[0].level_tag, "success")
        # switch user context
        self.client.logout()
