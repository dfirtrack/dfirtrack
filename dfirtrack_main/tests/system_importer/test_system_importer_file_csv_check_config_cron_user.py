...
class SystemImporterFileCsvCheckConfigCronUserViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        User.objects.create_user(
            username="testuser_system_importer_file_csv_check_config_cron_user",
            password="gRknQm6WAhvUc8uzYhbe",
        )
        User.objects.create_user(
            username="message_user", password="HmiOhvi5RNzrM8UAjy7v"
        )

        # change config
        set_csv_import_username(None)

    def test_system_importer_file_csv_check_config_cron_user_create_cron_no_import_user(
        self,
    ):
        """test importer view"""
        ...
        self.assertEqual(
            messages[0].message, "No user for import defined. Check config!"
        )
