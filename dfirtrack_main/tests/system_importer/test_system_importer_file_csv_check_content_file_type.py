...
class SystemImporterFileCsvCheckContentFileTypeViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_user = User.objects.create_user(
            ...
            password="3oKsgNPVdlmNPneLhdr9",
        )
        User.objects.create_user(
            username="message_user", password="a3ZEI74fr0lmA3pSh96b"
        )
    ...
    def test_system_importer_file_csv_cron_wrong_type(self):
        ...
        self.assertEqual(
            ...
            "testuser_system_importer_file_csv_check_content_file_type",
        )
        self.assertEqual(
            messages[0].message,
            "[Scheduled task CSV system importer] Wrong file type for CSV import. Check config or file system!",
        )
    ...
    def test_system_importer_file_csv_upload_post_wrong_type(self):
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                "dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_04_wrong_type.png",
            ),
            "rb",
        )
        # create post data
    ...
    def test_system_importer_file_csv_cron_corrupted(self):
        ...
        self.assertEqual(
            messages[0].message,
            "[Scheduled task CSV system importer] File is corrupted. Check config or file system!",
        )
        self.assertEqual(messages[0].level_tag, "error")
        # switch user context
        self.client.logout()
    ...
    def test_system_importer_file_csv_upload_post_corrupted(self):
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                "dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_05_corrupted.csv",
            )
        )
        # create post data
        data_dict = {
