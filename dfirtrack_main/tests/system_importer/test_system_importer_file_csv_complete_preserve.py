...
def pre_compare_system_and_attributes_csv(self):
    ...
    self.assertFalse(
        Recommendation.objects.filter(
            recommendation_name='recommendation_42_1'
        ).exists()
    )
    self.assertFalse(
        Serviceprovider.objects.filter(
            serviceprovider_name='serviceprovider_42_1'
        ).exists()
    )
    self.assertFalse(
        Systemtype.objects.filter(systemtype_name='systemtype_42_1').exists()
    )
    ...
    self.assertFalse(
        system_csv_42_001.company.filter(company_name='company_42_1').exists()
    )
    self.assertFalse(
        system_csv_42_002.company.filter(company_name='company_42_1').exists()
    )
    self.assertFalse(
        system_csv_42_003.company.filter(company_name='company_42_1').exists()
    )
...
class SystemImporterFileCsvCompletePreserveViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_user = User.objects.create_user(
            username='testuser_system_importer_file_csv_complete_preserve',
            password='pcLkWltExZwHFuE3vaeD',
        )
        User.objects.create_user(
            username='message_user', password='MLB6KZcrs2peudXc25Ix'
        )
    ...
    def test_system_importer_file_csv_complete_preserve_cron(self):
        ...
        # mock timezone.now()
        t_1 = datetime(2021, 3, 20, 18, 30, tzinfo=dttimezone.utc)
    ...
    def test_system_importer_file_csv_complete_preserve_upload_post(self):
        ...
        systemcsv = open(
            os.path.join(
                BASE_DIR,
                'dfirtrack_main/tests/system_importer/system_importer_file_csv_files/system_importer_file_csv_testfile_42_complete_preserve.csv',
            )
        )
        # create post data
        data_dict = {
