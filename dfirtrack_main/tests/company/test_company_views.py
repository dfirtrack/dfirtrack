...
class CompanyViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        User.objects.create_user(
            username="testuser_company", password="MbJfulGWGKeqceBtN9Mi"
        )
    ...
    def test_company_detail_not_logged_in(self):
        ...
        destination = "/login/?next=" + urllib.parse.quote(
            "/company/" + str(company_1.company_id) + "/", safe=""
        )
        # get response
        response = self.client.get(
            "/company/" + str(company_1.company_id) + "/", follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
    ...
    def test_company_edit_not_logged_in(self):
        ...
        # create url
        destination = "/login/?next=" + urllib.parse.quote(
            "/company/" + str(company_1.company_id) + "/edit/", safe=""
        )
        # get response
        response = self.client.get(
            "/company/" + str(company_1.company_id) + "/edit/", follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
    ...
    def test_company_edit_post_redirect(self):
        ...
        response = self.client.post(
            "/company/" + str(company_1.company_id) + "/edit/", data_dict
        )
        # get object
        company_2 = Company.objects.get(company_name="company_edit_post_test_2")
        # create url
        destination = urllib.parse.quote(
            "/company/" + str(company_2.company_id) + "/", safe="/"
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
