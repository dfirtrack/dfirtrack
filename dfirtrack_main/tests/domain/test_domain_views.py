...
class DomainViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        User.objects.create_user(
            username="testuser_domain", password="vOKJXW7ZsJ7TZ3dsu43w"
        )
    ...
    def test_domain_detail_not_logged_in(self):
        ...
        destination = "/login/?next=" + urllib.parse.quote(
            "/domain/" + str(domain_1.domain_id) + "/", safe=""
        )
        # get response
        response = self.client.get(
            "/domain/" + str(domain_1.domain_id) + "/", follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
    ...
    def test_domain_edit_not_logged_in(self):
        ...
        # create url
        destination = "/login/?next=" + urllib.parse.quote(
            "/domain/" + str(domain_1.domain_id) + "/edit/", safe=""
        )
        # get response
        response = self.client.get(
            "/domain/" + str(domain_1.domain_id) + "/edit/", follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
    ...
    def test_domain_edit_post_redirect(self):
        ...
        response = self.client.post(
            "/domain/" + str(domain_1.domain_id) + "/edit/", data_dict
        )
        # get object
        domain_2 = Domain.objects.get(domain_name="domain_edit_post_test_2")
        # create url
        destination = urllib.parse.quote(
            "/domain/" + str(domain_2.domain_id) + "/", safe="/"
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=302, target_status_code=200
