...
class Artifact(models.Model):
    ...
    def logger(artifact, request_user, log_text):  # coverage: ignore branch
        ...
        stdlogger.info(
            ...
            + "|artifactpriority:"
            + str(artifact.artifactpriority.artifactpriority_name)
            + "|artifactstatus:"
            + str(artifact.artifactstatus.artifactstatus_name)
            + "|artifacttype:"
            ...
            + str(artifact.artifact_note_analysisresult)
            + "|artifact_note_external:"
            + str(artifact.artifact_note_external)
            + "|artifact_note_internal:"
            + str(artifact.artifact_note_internal)
            + "|artifact_slug:"
            + str(artifact.artifact_slug)
            + "|artifact_requested_time:"
            + requestedtime
            + "|artifact_acquisition_time:"
            + acquisitiontime
            + "|artifact_md5:"
            + str(artifact.artifact_md5)
            + "|artifact_sha1:"
            + str(artifact.artifact_sha1)
            + "|artifact_sha256:"
            + str(artifact.artifact_sha256)
            + "|artifact_source_path:"
            + str(artifact.artifact_source_path)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:
            ...
            self.artifact_storage_path = self.create_artifact_storage_path(
                self.system.system_uuid,
                self.artifacttype.artifacttype_slug,
                self.artifact_uuid,
            )
        ...
        if (
            self.artifactstatus in artifactstatus_requested
            and self.artifact_requested_time == None
        ):
        ...
        if (
            self.artifactstatus in artifactstatus_acquisition
            and self.artifact_acquisition_time == None
        ):
    ...
    def check_existing_hashes(self, request):
        ...
        if self.artifact_md5:
            ...
            artifacts = Artifact.objects.filter(artifact_md5=self.artifact_md5).exclude(
        ...
        if self.artifact_sha1:
            ...
            artifacts = Artifact.objects.filter(
                artifact_sha1=self.artifact_sha1
...
class Artifactpriority(models.Model):
    ...
    def logger(artifactpriority, request_user, log_text):
        stdlogger.info(
            ...
            + log_text
            + " artifactpriority_id:"
            + str(artifactpriority.artifactpriority_id)
            + "|artifactpriority_name:"
            + str(artifactpriority.artifactpriority_name)
            + "|artifactpriority_note:"
            + str(artifactpriority.artifactpriority_note)
            + "|artifactpriority_slug:"
            + str(artifactpriority.artifactpriority_slug)
        )
...
class Artifactstatus(models.Model):
    ...
    def logger(artifactstatus, request_user, log_text):
        stdlogger.info(
            ...
            + log_text
            + " artifactstatus_id:"
            + str(artifactstatus.artifactstatus_id)
            + "|artifactstatus_name:"
            + str(artifactstatus.artifactstatus_name)
            + "|artifactstatus_note:"
            + str(artifactstatus.artifactstatus_note)
            + "|artifactstatus_slug:"
            + str(artifactstatus.artifactstatus_slug)
        )
...
class Artifacttype(models.Model):
    ...
    def logger(artifacttype, request_user, log_text):
        stdlogger.info(
            ...
            + log_text
            + " artifacttype_id:"
            + str(artifacttype.artifacttype_id)
            + "|artifacttype_name:"
            + str(artifacttype.artifacttype_name)
            + "|artifacttype_note:"
            + str(artifacttype.artifacttype_note)
            + "|artifacttype_slug:"
            + str(artifacttype.artifacttype_slug)
        )
