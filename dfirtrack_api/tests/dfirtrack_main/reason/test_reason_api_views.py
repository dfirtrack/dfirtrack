...
class ReasonAPIViewTestCase(TestCase):
    ...
    def test_reason_list_api_unauthorized(self):
        ...
        # get response
        response = self.client.get('/api/reasons/')
        # compare
    ...
    def test_reason_list_api_method_post(self):
        ...
        # get response
        response = self.client.post('/api/reasons/', data=poststring)
        # compare
    ...
    def test_reason_list_api_redirect(self):
        ...
        login = self.client.login(username='testuser_reason_api', password='tvjnIPBlhP9P3ixDHVE7')
        # create url
        destination = urllib.parse.quote('/api/reasons/', safe='/')
        # get response
        response = self.client.get('/api/reasons', follow=True)
        # compare
        self.assertRedirects(response, destination, status_code=301, target_status_code=200)

#    def test_reason_list_api_get_user_context(self):
#        """ test user context """
#
#        # login testuser
#        login = self.client.login(username='testuser_reason_api', password='tvjnIPBlhP9P3ixDHVE7')
...
#        response = self.client.get('/api/reasons/')
#        # compare
#        self.assertEqual(str(response.context['user']), 'testuser_reason_api')
    ...
    def test_reason_detail_api_unauthorized (self):
        ...
        # get response
        response = self.client.get('/api/reasons/' + str(reason_api_1.reason_id) + '/')
        # compare
        self.assertEqual(response.status_code, 401)
    ...
    def test_reason_detail_api_method_delete(self):
        ...
        login = self.client.login(username='testuser_reason_api', password='tvjnIPBlhP9P3ixDHVE7')
        # get response
        response = self.client.delete('/api/reasons/' + str(reason_api_1.reason_id) + '/')
        # compare
    ...
    def test_reason_detail_api_method_put(self):
        ...
        login = self.client.login(username='testuser_reason_api', password='tvjnIPBlhP9P3ixDHVE7')
        # create url
        destination = urllib.parse.quote('/api/reasons/' + str(reason_api_1.reason_id) + '/', safe='/')
        # create PUT string
    ...
    def test_reason_detail_api_redirect(self):
        ...
        destination = urllib.parse.quote('/api/reasons/' + str(reason_api_1.reason_id) + '/', safe='/')
        # get response
        response = self.client.get('/api/reasons/' + str(reason_api_1.reason_id), follow=True)
        # compare
        self.assertRedirects(response, destination, status_code=301, target_status_code=200)

#    def test_reason_detail_api_get_user_context(self):
#        """ test user context """
#
#        # get object
#        reason_api_1 = Reason.objects.get(reason_name='reason_api_1')
