...
class SystemuserAPIViewTestCase(TestCase):
    ...
    def test_systemuser_list_api_method_post(self):
        ...
        # get response
        response = self.client.post('/api/systemusers/', data=poststring)
        # compare
    ...
    def test_systemuser_list_api_redirect(self):
        ...
        login = self.client.login(username='testuser_systemuser_api', password='Yij2up4yTV2BU6x9xKZV')
        # create url
        destination = urllib.parse.quote('/api/systemusers/', safe='/')
        # get response
        response = self.client.get('/api/systemusers', follow=True)
        # compare
...
#        self.assertEqual(str(response.context['user']), 'testuser_systemuser_api')
    ...
    def test_systemuser_detail_api_method_delete(self):
        ...
        login = self.client.login(username='testuser_systemuser_api', password='Yij2up4yTV2BU6x9xKZV')
        # get response
        response = self.client.delete('/api/systemusers/' + str(systemuser_api_1.systemuser_id) + '/')
        # compare
    ...
    def test_systemuser_detail_api_method_put(self):
        ...
        login = self.client.login(username='testuser_systemuser_api', password='Yij2up4yTV2BU6x9xKZV')
        # create url
        destination = urllib.parse.quote('/api/systemusers/' + str(systemuser_api_1.systemuser_id) + '/', safe='/')
        # create PUT string
        putstring = {
