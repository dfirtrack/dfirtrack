...
class ContactAPIViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        User.objects.create_user(
            username="testuser_contact_api", password="tvjnIPBlhP9P3ixDHVE7"
        )
    ...
    def test_contact_detail_api_unauthorized(self):
        ...
        response = self.client.get(
            "/api/contact/" + str(contact_api_1.contact_id) + "/"
        )
        # compare
        self.assertEqual(response.status_code, 401)
    ...
    def test_contact_detail_api_method_put(self):
        ...
        destination = urllib.parse.quote(
            "/api/contact/" + str(contact_api_1.contact_id) + "/", safe="/"
        )
        # create PUT string
        putstring = {
    ...
    def test_contact_detail_api_redirect(self):
        ...
        # get response
        response = self.client.get(
            "/api/contact/" + str(contact_api_1.contact_id), follow=True
        )
        # compare
        self.assertRedirects(
            response, destination, status_code=301, target_status_code=200
